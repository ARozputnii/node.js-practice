...
csvData.forEach(async (line) => {
  ...
  athleteID = await athleteID.setRecord(
    'INSERT INTO athletes (full_name, year_of_birth, sex, params, team_id) ' +
      `VALUES ('${athleteData.Name}', '${athleteData.Age}', '${athleteData.Sex}', '${JSON.stringify(athleteData.Params)}', '${team || team.id}')`
  )

  const game = await DbConnector.getRecord(
    'SELECT * FROM games ' +
    `WHERE year="${gameData.Year}" AND season="${gameData.Season}" AND city="${gameData.City}"`
  )
  const gameCity = game.record ? game.record.city : undefined
  let gameID = game.record ? game.record.id : undefined
  gameID = await game.setRecord(
